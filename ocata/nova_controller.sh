...
openstack user create --domain default \
  --password $OPENSTACK_NOVA_USER_PASSWORD nova
...
openstack endpoint create --region $REGION_NAME \
  compute public http://$CH_M_INTERFACE_ADDRESS:8774/v2.1/%\(tenant_id\)s

openstack endpoint create --region $REGION_NAME \
  compute internal http://$CH_C_INTERFACE_ADDRESS:8774/v2.1/%\(tenant_id\)s

openstack endpoint create --region $REGION_NAME \
  compute admin http://$CH_C_INTERFACE_ADDRESS:8774/v2.1/%\(tenant_id\)s

#5.Create a Placement service user using your chosen PLACEMENT_PASS:
openstack user create --domain default \
  --password $OPENSTACK_PLACEMENT_USER_PASSWORD placement
...
openstack endpoint create --region $REGION_NAME placement public http://$CH_M_INTERFACE_ADDRESS:8778

openstack endpoint create --region $REGION_NAME placement public http://$CH_C_INTERFACE_ADDRESS:8778
...
sed -i "s/connection=sqlite:\/\/\/\/var\/lib\/nova\/nova.sqlite/connection = mysql+pymysql:\/\/nova:$NOVA_DB_PASSWORD@$CH_C_INTERFACE_ADDRESS\/nova_api/g" /etc/nova/nova.conf

sed -i "s/#connection=<None>/connection = mysql+pymysql:\/\/nova:$NOVA_DB_PASSWORD@$CH_C_INTERFACE_ADDRESS\/nova/g" /etc/nova/nova.conf
...
sed -i "s/#my_ip=10.89.104.70/my_ip= $CH_C_INTERFACE_ADDRESS/g" /etc/nova/nova.conf
...
sed -i "s/#vncserver_listen=127.0.0.1/vncserver_listen = $CH_C_INTERFACE_ADDRESS/g" /etc/nova/nova.conf
sed -i "s/#vncserver_proxyclient_address=127.0.0.1/vncserver_proxyclient_address = $CH_C_INTERFACE_ADDRESS/g" /etc/nova/nova.conf
